...
% ef;
% close all; imagesc(R); ef;
% spk_rate = conv(spk_mat,v);

% compare with raster
% ax = np;
% plt.raster2(spikes,[],[],ax,0,'k-','LineWidth',0.2);
% export_fig spikes.png -m3

% % sparse spikes - no need for now
% sparse_mat = sparse(n_cells,numel(t_mat));
% for icell = 1:n_cells
% 	sparse_mat(icell,ismember(t_mat,spikes{icell}*1e3)) = 1;
% end
% sparse_rate = sconv2(sparse_mat,v);
