function raster_smooth(spikes,toi,ax,varargin)
...
addParameter(p,'bin_width',20); % color of maximum firing
parse(p,varargin{:});
...
[R,~,~,tbin] = running_average(t_mat,spk_mat,p.Results.bin_width,p.Results.bin_width);
R = (R ./ max(R,[],1))';
R(isnan(R)) = 0;
n_color = 255;
